#!/usr/bin/env python3
"""
Построение рентгеновского изображения по прямым регистрациям рентгеновских/электронных частиц
на кремниевом детекторе (без сцинтиллятора).

Рентгеновские фотоны проходят через золотые полоски, поглощаются в Si-детекторе,
регистрируются напрямую.

Запуск из папки build:
    python3 build_xray_direct.py
"""

import numpy as np
import matplotlib.pyplot as plt
from scipy.ndimage import gaussian_filter
import pandas as pd
import re
import os
import sys

# ==================== Чтение параметров из global_parameters.cc ====================
def read_global_parameters():
    """Читает pixelSize, gridSize, slitWidth из global_parameters.cc"""
    params = {'pixelSize': 10.0, 'gridSize': 100, 'slitWidth': 50.0}
    
    script_dir = os.path.dirname(os.path.abspath(__file__))
    cc_file = os.path.join(script_dir, '..', 'src', 'global_parameters.cc')
    
    if not os.path.exists(cc_file):
        print(f"WARNING: global_parameters.cc not found: {cc_file}")
        return params
    
    with open(cc_file, 'r', encoding='utf-8') as f:
        content = f.read()
    
    match = re.search(r'pixelSize\s*=\s*([\d.]+)', content)
    if match: params['pixelSize'] = float(match.group(1))
    
    match = re.search(r'gridSize\s*=\s*(\d+)', content)
    if match: params['gridSize'] = int(match.group(1))
    
    match = re.search(r'slitWidth\s*=\s*([\d.]+)', content)
    if match: params['slitWidth'] = float(match.group(1))
    
    return params


# ==================== Чтение данных ====================
def read_hits_data(filename='hits_data.csv'):
    """Читает hits_data.csv, возвращает DataFrame"""
    for encoding in ['utf-8', 'utf-8-sig', 'latin1', 'cp1251']:
        try:
            df = pd.read_csv(filename, sep='\t', comment='#', encoding=encoding, low_memory=False)
            required_cols = ['Energy_eV', 'PosX_um', 'PosY_um', 'Type']
            if all(col in df.columns for col in required_cols):
                df['Energy_eV'] = pd.to_numeric(df['Energy_eV'], errors='coerce')
                df['PosX_um'] = pd.to_numeric(df['PosX_um'], errors='coerce')
                df['PosY_um'] = pd.to_numeric(df['PosY_um'], errors='coerce')
                df.dropna(subset=required_cols, inplace=True)
                return df[required_cols]
        except Exception as e:
            print(f"  Encoding {encoding}: {e}")
            continue
    
    return pd.DataFrame()


# ==================== Построение геометрии ====================
def build_geometry_mask(params, grid_size, x_edges, y_edges):
    """Создаёт бинарную маску золотых полосок.
    
    Полоски в C++ идут ВДОЛЬ оси Y (длина по Y = leadSize, ширина по X = slitWidth).
    np.histogram2d возвращает counts[i, j] для x_edges[i] и y_edges[j].
    geometry[i, j] = 1.0 если пиксель попадает под полоску.
    После .T: geometry_T[j, i] — строка по Y, столбец по X.
    """
    pixel_size = params['pixelSize']
    slit_width = params['slitWidth']
    lead_size = pixel_size * grid_size
    slit_period = slit_width * 2  # ширина + зазор (равные)
    num_slits = int(lead_size / slit_period)
    total_width = num_slits * slit_period
    start_x = -total_width / 2.0 + slit_width / 2.0
    
    x_centers = (x_edges[:-1] + x_edges[1:]) / 2.0
    y_centers = (y_edges[:-1] + y_edges[1:]) / 2.0
    
    geometry = np.zeros((grid_size, grid_size), dtype=float)
    
    # Полоски идут ВДОЛЬ Y, поэтому проверяем только X (1-я ось counts)
    for i in range(num_slits):
        strip_center = start_x + i * slit_period
        x_min = strip_center - slit_width / 2.0
        x_max = strip_center + slit_width / 2.0
        is_in_strip = (x_centers >= x_min) & (x_centers < x_max)
        
        # Помечаем строку (все Y) для этого X (1-я ось)
        for j, in_strip in enumerate(is_in_strip):
            if in_strip:
                geometry[j, :] = 1.0  # золото
    
    return geometry, num_slits, slit_width, slit_period, start_x


# ==================== Построение изображений ====================
def build_images(df, params):
    """Строит все изображения и сохраняет"""
    
    pixel_size = params['pixelSize']
    grid_size = params['gridSize']
    lead_size = pixel_size * grid_size
    
    # --- Фильтрация рентгеновских и электронных частиц ---
    # Регистрируем гамма, электроны, позитроны
    particle_types = ['gamma', 'e-', 'e+']
    mask = df['Type'].isin(particle_types)
    df_direct = df[mask].copy()
    
    print(f"\nTotal particles: {len(df):,}")
    print(f"Registered (gamma/e-/e+): {len(df_direct):,}")
    
    if len(df_direct) == 0:
        print("ERROR: No registered particles!")
        print(f"Available types: {df['Type'].unique()}")
        print(f"Energy range: {df['Energy_eV'].min():.1f}-{df['Energy_eV'].max():.1f} eV")
        return
    
    # --- Диапазон координат ---
    x_range = (-lead_size / 2, lead_size / 2)
    y_range = (-lead_size / 2, lead_size / 2)
    
    # --- 2D гистограмма (количество) ---
    print("\nBinning...")
    counts, x_edges, y_edges = np.histogram2d(
        df_direct['PosX_um'].values,
        df_direct['PosY_um'].values,
        bins=grid_size,
        range=[x_range, y_range]
    )
    
    # counts[i, j] соответствует x_centers[i] и y_centers[j]
    # Но imshow отображает первую ось по вертикали, вторую по горизонтали
    # Чтобы X был горизонтален, а Y вертикален — транспонируем
    counts_T = counts.T  # Теперь counts_T[j, i] соответствует y_centers[j] и x_centers[i]
    
    # Строим маску и транспонируем
    geometry_T, num_slits, slit_width, slit_period, start_x = build_geometry_mask(
        params, grid_size, x_edges, y_edges
    )
    geometry_T = geometry_T.T
    
    print(f"  Non-zero pixels: {np.sum(counts_T > 0):,} / {grid_size*grid_size:,}")
    print(f"  Max particles/pixel: {int(np.max(counts_T))}")
    print(f"  Mean energy: {np.mean(df_direct['Energy_eV']):.1f} eV")
    
    # --- Вычисление ослабления ---
    # Находим фоновые пиксели (без золота) для референса
    bg_mask = geometry_T == 0
    if np.sum(bg_mask) > 0:
        mean_bg_counts = np.mean(counts_T[bg_mask])
    else:
        mean_bg_counts = np.max(counts_T)
    
    # Аттенюация: I/I0 = counts / mean_bg_counts
    # A = -ln(I/I0)
    signal = np.where(counts_T > 0, counts_T / mean_bg_counts, 0)
    attenuation = -np.log(np.clip(signal, 1e-10, None))
    
    # --- Сглаживание ---
    counts_T_smooth = gaussian_filter(counts_T.astype(float), sigma=0.8)
    attenuation_smooth = gaussian_filter(attenuation, sigma=0.8)
    
    # ==================== Визуализация ====================
    x_centers = (x_edges[:-1] + x_edges[1:]) / 2.0
    y_centers = (y_edges[:-1] + y_edges[1:]) / 2.0
    extent = [x_range[0], x_range[1], y_range[0], y_range[1]]
    
    fig, axes = plt.subplots(1, 4, figsize=(24, 6))
    
    # 1) Карта количества частиц
    im1 = axes[0].imshow(
        counts_T_smooth, origin='lower', extent=extent,
        cmap='hot', interpolation='bilinear'
    )
    axes[0].set_xlabel('X, um', fontsize=12)
    axes[0].set_ylabel('Y, um', fontsize=12)
    axes[0].set_title('Particle count map', fontsize=13)
    plt.colorbar(im1, ax=axes[0], label='Particles/pixel')
    
    # 2) Аттенюация (рентгеновское изображение)
    att_finite = attenuation_smooth[~np.isinf(attenuation_smooth)]
    if len(att_finite) > 0:
        att_vmin = np.percentile(att_finite, 2)
        att_vmax = np.percentile(att_finite, 98)
    else:
        att_vmin, att_vmax = 0, 1
    
    im2 = axes[1].imshow(
        attenuation_smooth, origin='lower', extent=extent,
        cmap='gray_r', interpolation='bilinear',
        vmin=att_vmin, vmax=att_vmax
    )
    axes[1].set_xlabel('X, um', fontsize=12)
    axes[1].set_ylabel('Y, um', fontsize=12)
    axes[1].set_title(f'X-ray image (attenuation)\nDirect detection, sigma=0.8', fontsize=13)
    plt.colorbar(im2, ax=axes[1], label='A = -ln(I/I0)')
    
    # 3) Отношение сигнал/фон
    ratio = np.where(bg_mask, counts_T_smooth / mean_bg_counts, np.nan)
    im3 = axes[2].imshow(
        ratio, origin='lower', extent=extent,
        cmap='viridis', interpolation='bilinear',
        vmin=0, vmax=1.2
    )
    axes[2].set_xlabel('X, um', fontsize=12)
    axes[2].set_ylabel('Y, um', fontsize=12)
    axes[2].set_title('Signal/Background ratio\n(gold = dark)', fontsize=13)
    plt.colorbar(im3, ax=axes[2], label='I/I0')
    
    # 4) Геометрия (золотые полоски)
    im4 = axes[3].imshow(
        geometry_T, origin='lower', extent=extent,
        cmap='Greys', interpolation='nearest',
        vmin=0, vmax=1
    )
    axes[3].set_xlabel('X, um', fontsize=12)
    axes[3].set_ylabel('Y, um', fontsize=12)
    axes[3].set_title(f'Geometry: {num_slits} gold strips\nwidth={slit_width} um, period={slit_period} um', fontsize=13)
    plt.colorbar(im4, ax=axes[3], label='Gold / Gap')
    
    plt.tight_layout()
    plt.savefig('xray_direct_full.png', dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
    print("\nSaved: xray_direct_full.png")
    
    # ==================== Отдельные крупные изображения ====================
    
    # Аттенюация (крупно)
    fig, ax = plt.subplots(1, 1, figsize=(10, 10))
    im = ax.imshow(
        attenuation_smooth, origin='lower', extent=extent,
        cmap='gray_r', interpolation='bilinear',
        vmin=att_vmin, vmax=att_vmax
    )
    ax.set_xlabel('X, um', fontsize=14)
    ax.set_ylabel('Y, um', fontsize=14)
    ax.set_title(f'X-ray image (direct detection)\nDirect Si detection, sigma=0.8', fontsize=16)
    cbar = plt.colorbar(im, ax=ax, label='A = -ln(I/I0)')
    plt.tight_layout()
    plt.savefig('xray_direct_attenuation.png', dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
    print("Saved: xray_direct_attenuation.png")
    
    # Карта количества (крупно)
    fig, ax = plt.subplots(1, 1, figsize=(10, 10))
    im = ax.imshow(
        counts_T_smooth, origin='lower', extent=extent,
        cmap='hot', interpolation='bilinear'
    )
    ax.set_xlabel('X, um', fontsize=14)
    ax.set_ylabel('Y, um', fontsize=14)
    ax.set_title(f'Particle count\nDirect Si detection, sigma=0.8', fontsize=16)
    cbar = plt.colorbar(im, ax=ax, label='Particles/pixel')
    plt.tight_layout()
    plt.savefig('xray_direct_counts.png', dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
    print("Saved: xray_direct_counts.png")
    
    # ==================== Статистика ====================
    print(f"\n{'='*55}")
    print("STATISTICS:")
    print(f"  Pixels: {grid_size}x{grid_size} = {grid_size*grid_size:,}")
    print(f"  Non-zero: {np.sum(counts_T > 0):,} ({100*np.sum(counts_T>0)/(grid_size*grid_size):.1f}%)")
    print(f"  Total registered: {len(df_direct):,}")
    print(f"  Max/pixel: {int(np.max(counts_T))}")
    print(f"  Mean/pixel (non-zero): {np.mean(counts_T[counts_T>0]):.1f}")
    print(f"  Mean (background, no gold): {mean_bg_counts:.1f}")
    print(f"  Attenuation max: {np.max(attenuation_smooth):.3f}")
    
    # Статистика по золоту и фону
    gold_mask = geometry_T == 1
    if np.sum(gold_mask) > 0 and np.sum(bg_mask) > 0:
        mean_gold_counts = np.mean(counts_T[gold_mask])
        mean_bg_counts_val = np.mean(counts_T[bg_mask])
        contrast = 1 - mean_gold_counts / mean_bg_counts_val
        print(f"\n  Mean (gold): {mean_gold_counts:.1f}")
        print(f"  Mean (background): {mean_bg_counts_val:.1f}")
        print(f"  Contrast: {contrast*100:.1f}%")
        print(f"  Ratio I_gold/I_bg: {mean_gold_counts/mean_bg_counts_val:.3f}")
    print(f"{'='*55}")
    
    # ==================== Сохранение данных ====================
    np.savez('xray_direct_data.npz',
             counts=counts_T,
             counts_smooth=counts_T_smooth,
             attenuation=attenuation,
             attenuation_smooth=attenuation_smooth,
             geometry=geometry_T,
             x_edges=x_edges,
             y_edges=y_edges,
             mean_bg_counts=mean_bg_counts,
             num_slits=num_slits,
             slit_width=slit_width,
             slit_period=slit_period,
             start_x=start_x)
    print("Saved: xray_direct_data.npz")


# ==================== Основная функция ====================
def main():
    print("="*55)
    print("X-RAY IMAGE (DIRECT DETECTION)")
    print("="*55)
    
    # Параметры
    print("\nReading parameters...")
    params = read_global_parameters()
    print(f"  pixelSize = {params['pixelSize']} um")
    print(f"  gridSize  = {params['gridSize']}")
    print(f"  slitWidth = {params['slitWidth']} um")
    
    # Данные
    print("\nReading hits_data.csv...")
    df = read_hits_data('hits_data.csv')
    print(f"  Loaded records: {len(df):,}")
    
    if len(df) == 0:
        print("ERROR: Could not read data!")
        sys.exit(1)
    
    # Построение
    print("\n" + "="*55)
    build_images(df, params)
    print("\nDone!")


if __name__ == '__main__':
    main()
